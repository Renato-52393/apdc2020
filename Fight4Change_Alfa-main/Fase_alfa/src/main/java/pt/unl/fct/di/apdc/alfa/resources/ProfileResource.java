...
public class ProfileResource {
	...
	public Response changeRoles(@PathParam("user") String username1, RegisterData data) {
		...
		Key logKey = datastore.newKeyFactory().addAncestors(PathElement.of("User", data.username)).setKind("UserLog")
		...
		try {
			...
			if (!user2.getString("role").equals(USER)) {
				...
				return Response.status(Status.BAD_REQUEST)
						.entity("Do not have permission to change '" + data.username + "' role.").build();
			}
			...
			if (log != null) {
				log = Entity.newBuilder(logKey).set("Token_username", log.getString("Token_username"))
						.set("Token_id", log.getString("Token_id")).set("role", data.role)
						.set("Token_creation", log.getLong("Token_creation"))
						.set("Token_expiration", log.getLong("Token_expiration")).build();

				txn.put(log);
			}
			user2 = Entity.newBuilder(userKey2).set("user_name", user2.getString("user_name"))
					.set("user_pwd", user2.getString("user_pwd")).set("user_email", user2.getString("user_email"))
					.set("user_role", data.role).set("user_account_status", user2.getString("user_account_status"))
					...
					.set("user_gender", user2.getString("user_gender")).build();
	...
	public Response changeAttributes(RegisterData data, @Context HttpServletRequest request,
		...
		/*
		 * if(log == null) { return
		 * Response.status(Status.BAD_REQUEST).entity("User is not logged in.").build();
		 * } if (!data.validPassword()) return
		 * Response.status(Status.BAD_REQUEST).entity("Invalid password").build();
		 */
		...
		try {
			...
			user = Entity.newBuilder(userKey)
					...
					// .set("user_phone", data.phone.equals("") ? user.getString("user_phone") :
					// data.phone)
					...
					// .set("user_dateofbirth", data.dateofbirth.equals("") ?
					// user.getString("user_dateofbirth") : data.dateofbirth)
					// .set("user_gender", data.gender.equals("") ? user.getString("user_gender") :
					// data.gender)
					.build();
	...
	public Response getProfile(@Context HttpServletRequest request,
									 @Context HttpHeaders headers) {
		LOG.fine("Change of attributes attempt");
		...
		try {
			...
			RegisterData data = new RegisterData(user.getString("user_email"), user.getString("user_name"), null, null, user.getString("user_address"),
					user.getString("user_zipcode"), user.getString("user_role"), null);
